...
#Will update to use StratifiedKFold (with shuffling)
kf = KFold(n_splits = n_fold, shuffle = False, random_state = 44)

train_indices = []
validation_indices = []

#Set training/validation indices
for train_index, validation_index in kf.split(reviews):
...
for fold in range(n_fold):
    ...
    #Instantiate model
    lm = SGDRegressor(loss = 'squared_loss', n_iter = np.ceil(10**6 /train_indices[fold].shape[0]), alpha = 10**-10,
                          l1_ratio = 1, random_state = 44, verbose = 3)
